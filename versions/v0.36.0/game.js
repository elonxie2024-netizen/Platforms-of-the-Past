...
const customLevelDetailsMenu = document.querySelector("#customLevelDetailsMenu");
const customLevelDetailsTitle = document.querySelector("#customLevelDetailsTitle");
const customLevelDetailsCreator = document.querySelector("#customLevelDetailsCreator");
const customLevelDetailsMeta = document.querySelector("#customLevelDetailsMeta");
...
function publishedInputMask() {
  return Number(input.left) | Number(input.right) << 1 | Number(input.jump) << 2 |
    Number(input.down) << 3 | Number(input.rewind) << 4 | Number(input.forwardTime) << 5;
}

function recordPublishedInputTransition() {
  if (!publishedRunEvidence || publishedRunEvidence.ended) return;
...
function recordPublishedRunState(force = false) {
  ...
  publishedRunEvidence.lastSampleAt = elapsed;
  if (!publishedRunEvidence.initialState) {
    publishedRunEvidence.initialState = {
      x: Math.round(player.x * 10) / 10,
      y: Math.round(player.y * 10) / 10,
      objectCount: currentLevel().platforms.length + (currentLevel().enemies || []).length
    };
  }
  let objects = [];
  if (force || Math.floor(elapsed / 2) !== Math.floor((elapsed - .25) / 2)) {
    ...
    (currentLevel().enemies || []).forEach((enemy, index) => objects.push([`e${index}`, Math.round(enemy.x), Math.round(enemy.y), enemy.alive ? 1 : 0]));
    (currentLevel().switches || []).forEach((levelSwitch, index) => objects.push([`s${index}`, Math.round(levelSwitch.x), Math.round(levelSwitch.y), levelSwitch.flipped ? 1 : 0]));
    (currentLevel().pressurePlates || []).forEach((plate, index) => objects.push([`p${index}`, Math.round(plate.x), Math.round(plate.y), plate.active ? 1 : 0]));
    if (echo) objects.push(["echo", Math.round(echo.x), Math.round(echo.y), 1]);
    if (objects.length > 80) {
      const start = Math.floor(elapsed) * 80 % objects.length;
      objects = [...objects.slice(start, start + 80), ...objects.slice(0, Math.max(0, start + 80 - objects.length))];
    }
  }
...
function publishedReplayData() {
  ...
  const expanded = {
    ...
    levelDigest: window.PlatformsReplayValidator?.levelDigest(publishedLevelContext?.levelData || {}) || "",
    initialState: publishedRunEvidence?.initialState,
    inputEvents: publishedRunEvidence?.inputEvents || [],
...
function normalizedPlayerPreferences(value = {}) {
  ...
  return {
    ...
    audioMix,
    displaySize: value.displaySize === "small" ? "small" : "large",
...
function drawGameArt(name, x, y, width, height, targetContext = ctx) {
...
function startEditorPlaytest(levelData, options = {}) {
  ...
  publishedLevelContext = publishedLevelActive ? {
    ...
    levelData: options.levelData ? window.PlatformsLevelData.cloneLevel(options.levelData).level : null,
    startingDeaths: 0
  } : null;
  resetPublishedRunEvidence();
...
function recordPublishedLevelClear() {
  if (!publishedLevelActive || !publishedLevelContext || !publishedRunEvidence || publishedRunEvidence.ended) return;
  const context = { ...publishedLevelContext };
  ...
  const flyEver = publishedRunEvidence.flyEver;
  const cheatEver = publishedRunEvidence.cheatEver;
...
function hasAuthReturnParameters() {
  ...
  return Boolean(url.hash) || ["code", "error", "error_code", "token_hash", "type"].some(key => url.searchParams.has(key));
}
...
function showAccountMode(mode, message = "") {
  ...
  if (mode === "profile") {
    profileDisplayName.value = accountDisplayName();
    profileUsername.value = accountProfile?.username || "";
  }
...
function cleanAuthReturnUrl() {
  if (!hasAuthReturnParameters()) return;
  try {
    const clean = new URL(accountRedirectUrl());
    const publicLevelId = new URL(location.href).searchParams.get("level");
    if (publicLevelId) clean.searchParams.set("level", publicLevelId);
    history.replaceState({}, document.title, clean.href);
  }
...
async function syncProgressWithAccount(session, event = "SIGNED_IN") {
  ...
  await window.PlatformsEditor?.setAccountContext({
    ...
    displayName: service.cleanDisplayName(session.user.user_metadata?.display_name) || `Traveler-${session.user.id.slice(0, 6)}`,
    service
  });
  ...
  await window.PlatformsEditor?.setAccountContext({ userId, displayName: accountDisplayName(), service });
  ...
  } else if (event === "SIGNED_IN" && hasAuthReturnParameters()) {
...
async function initializeAccounts() {
  ...
  try {
    const session = await window.PlatformsAccount.initialize(SUPABASE_URL, SUPABASE_PUBLISHABLE_KEY);
    await window.PlatformsEditor?.setAccountContext({
      userId: session?.user?.id || null,
      displayName: session?.user ? (window.PlatformsAccount.cleanDisplayName(session.user.user_metadata?.display_name) || `Traveler-${session.user.id.slice(0, 6)}`) : "Guest",
...
function renderPublicProfile(data) {
  ...
  data.levels.forEach((level) => {
    ...
    play.type = "button";
    play.textContent = "Details";
    play.addEventListener("click", () => openCustomLevelDetails(level.level_id, "profile"));
    details.append(title, meta);
...
async function openPublicProfile(userId, source = "main") {
  ...
  else if (source === "details") customLevelDetailsMenu.hidden = true;
...
function closePublicProfile() {
  ...
  if (publicProfileReturn === "community") {
    ...
    leaderboardMenu.hidden = false;
    closeLeaderboardButton.focus();
  } else if (publicProfileReturn === "details") {
    customLevelDetailsMenu.hidden = false;
    customLevelDetailsCreator.focus();
  } else {
    mainMenu.hidden = false;
...
async function openCustomLevelDetails(levelId, returnTo = "community") {
  ...
  communityLevelsMenu.hidden = true;
  if (returnTo === "profile") publicProfileMenu.hidden = true;
  if (returnTo === "pause") pauseMenu.hidden = true;
  mainMenu.hidden = true;
  customLevelDetailsMenu.hidden = false;
  customLevelDetailsTitle.textContent = "Loading level...";
  ...
  try {
    ...
    if (!entry) throw new Error("Published level not found.");
    customLevelDetailsEntry = entry;
    customLevelDetailsTitle.textContent = entry.level_name || "Untitled Level";
    customLevelDetailsCreator.textContent = `${entry.owner_name || "Unknown creator"} · @${entry.owner_username || "unknown"}`;
    customLevelDetailsCreator.disabled = !entry.owner_id;
    customLevelDetailsMeta.textContent = `Published ${formatCommunityDate(entry.published_at)} · Updated ${formatCommunityDate(entry.updated_at)}`;
...
function renderCustomLevelRuns(runs) {
  ...
  runs.forEach(run => {
    ...
    name.textContent = run.runner_name;
    const time = document.createElement("span");
    time.textContent = formatRunTime(Number(run.seconds));
    const status = document.createElement("small");
    const stateLabel = validationState === "pending" || validationState === "processing"
      ? "Pending trusted verification"
      : validationState === "legacy"
...
function closeCustomLevelDetails() {
  customLevelDetailsRequest++;
  customLevelDetailsMenu.hidden = true;
  if (customLevelDetailsReturn === "community") {
    communityLevelsMenu.hidden = false;
    communitySearchInput.focus();
  } else if (customLevelDetailsReturn === "pause") {
    pauseMenu.hidden = false;
    pauseLeaderboardButton.focus();
  } else if (customLevelDetailsReturn === "profile") {
    publicProfileMenu.hidden = false;
    publicProfileLevels.querySelector("button")?.focus();
  } else {
    mainMenu.hidden = false;
    communityLevelsButton.focus();
  }
...
function levelUsesRewindField() {
  return Boolean(currentLevel().rewindField ||
...
function toggleEchoRecording() {
  if (!levelSupportsEcho()) return false;
  const action = echoRecording ? "echo-stop" : echoPreview ? "echo-create" : "echo-record";
  const changed = echoRecording ? finishEchoRecording() : echoPreview ? createEchoFromPreview() : beginEchoRecording();
  if (changed) recordPublishedAction(action);
  return changed;
}
...
function destroyEcho() {
  ...
  echo = null;
  recordPublishedAction("echo-destroy");
  playSfx("switch", .45);
  return true;
}
...
function activateNearbySwitch(actor = player) {
  ...
  if (actor === player) recordPublishedAction("interact");
  playSfx("switch");
  return true;
}
...
function setFlightEnabled(enabled) {
  if (enabled) markPublishedCheatUsed(true);
...
function setCollisionDisabled(enabled) {
  if (enabled) markPublishedCheatUsed(false, "collision");
...
function setInvincibilityEnabled(enabled) {
  if (enabled) markPublishedCheatUsed(false, "invincibility");
...
customLevelDetailsCreator.addEventListener("click", () => {
...
pauseLeaderboardButton.addEventListener("click", () => {
  if (publishedLevelActive && publishedLevelContext) openCustomLevelDetails(publishedLevelContext.levelId, "pause");
  else openLeaderboard("pause");
});
...
signUpForm.addEventListener("submit", async (event) => {
  ...
  const username = window.PlatformsAccount?.cleanUsername(formData.get("username"));
  ...
  try {
    const result = await window.PlatformsAccount.signUp(email, password, displayName, username, accountRedirectUrl());
    signUpForm.reset();
...
profileForm.addEventListener("submit", async (event) => {
  ...
  try {
    accountProfile = await window.PlatformsAccount.updateProfile(accountSession.user.id, displayName, username);
    renderAccountState("Profile updated.");
    accountMenu.hidden = true;
  } catch (error) {
    setAccountMessage(accountFriendlyError(error), true);
  } finally {
...
const requestFullscreen = gameFrame.requestFullscreen?.bind(gameFrame)
  || gameFrame.webkitRequestFullscreen?.bind(gameFrame);
...
async function toggleFullscreen() {
  try {
    if (fullscreenElement()) await exitFullscreen?.();
      ...
      fullscreenBaseWidth = gameSurface.offsetWidth;
      fullscreenBaseHeight = gameSurface.offsetHeight;
...
let fullscreenBaseWidth = gameSurface.offsetWidth;
let fullscreenBaseHeight = gameSurface.offsetHeight;
const FULLSCREEN_MARGIN = 12;

function syncFullscreenUiScale() {
  if (fullscreenElement() !== gameFrame) {
    gameFrame.style.removeProperty("--fullscreen-ui-scale");
    gameFrame.style.removeProperty("--fullscreen-surface-width");
    gameFrame.style.removeProperty("--fullscreen-surface-height");
    gameFrame.style.removeProperty("--fullscreen-offset-x");
    gameFrame.style.removeProperty("--fullscreen-offset-y");
    return;
  }
  const availableWidth = Math.max(1, gameFrame.clientWidth - FULLSCREEN_MARGIN * 2);
  const availableHeight = Math.max(1, gameFrame.clientHeight - FULLSCREEN_MARGIN * 2);
  const scale = Math.min(
    availableWidth / Math.max(1, fullscreenBaseWidth),
    availableHeight / Math.max(1, fullscreenBaseHeight)
  );
  const offsetX = (gameFrame.clientWidth - fullscreenBaseWidth * scale) / 2;
  const offsetY = (gameFrame.clientHeight - fullscreenBaseHeight * scale) / 2;
  gameFrame.style.setProperty("--fullscreen-ui-scale", String(scale));
  gameFrame.style.setProperty("--fullscreen-surface-width", `${fullscreenBaseWidth}px`);
  gameFrame.style.setProperty("--fullscreen-surface-height", `${fullscreenBaseHeight}px`);
  gameFrame.style.setProperty("--fullscreen-offset-x", `${offsetX}px`);
  gameFrame.style.setProperty("--fullscreen-offset-y", `${offsetY}px`);
}
...
function updateDisplayScale() {
  const shouldScale = displaySize === "large" && !fullscreenElement();
...
function drawSprite(index, x, y, w, h, targetContext = ctx) {
  ...
  targetContext.drawImage(spriteSheet, sx, sy, sw, sh, x, y, w, h);
  return true;
}
...
window.PlatformsGamePreview = Object.freeze({ drawAssetRectangle, drawSprite, drawGameArt });
...
function drawRewindPathPreview(time) {
  if (!currentLevel().rewindChapter) return;
