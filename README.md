...
**Watch trusted runs.** Published-level leaderboards offer Watch for trusted valid runs on the current immutable version. Playback drives the existing level simulation from recorded controls without awarding progress or submitting a run. Signed-in players can also start a normal ranked attempt while a translucent, non-interactive replay ghost follows the trusted run.

**Choose a custom-level type.** Creators can publish Exit or Exit + Required Stars levels. Every immutable publication version becomes verified independently after its first valid cheat-free completion. Version 0.40.0 permanently removed Survival and irreversibly purges all Survival drafts and derived cloud data when the full setup runs.
...
**Inspect published levels before playing.** Community Levels and creator profiles open a public detail screen with creator attribution, level type, objective, immutable version, verification status, the signed-in player's best trusted result, and a fastest-time leaderboard for that exact version. The full level snapshot is downloaded only after Play is selected, and existing direct-play links still launch immediately.

**Trust what the detail screen shows.** Version 0.36.1 rechecks the current immutable publication before Play and asks the player to review fresh details if the creator has published a newer version. Level metadata and Play remain available when leaderboard requests fail, with separate loading, retry, and empty states. Long names and leaderboard rows remain readable at both display sizes.
...
**Run regression tests.** From PowerShell in the repository folder, run `powershell -ExecutionPolicy Bypass -File .\tests\run-tests.ps1`. The dependency-free suite launches Chrome or Edge headlessly and tests the two supported custom-level types, Survival rejection, trusted replay derivation, compact replay bounds, forged claims, immutable publications, exact-version fastest-time leaderboards, private Favorites, metadata-only detail queries, serialization, save codes, the complete game boot path, and matching database/source contracts without manual browser interaction. GitHub Actions runs the same command automatically on pushes and pull requests.
