...
values ('intro-seven-v1', 'Version 0.6.2 to 0.9.1', array['v0.8.0', 'v0.8.1', 'v0.8.3', 'v0.9.0', 'v0.9.1'])
...
create table if not exists public.leaderboard_scores (
  ...
  stars smallint not null check (stars between 0 and 36),
  score numeric(8,1) generated always as (round(300 - seconds + stars * 2, 1)) stored,
  splits jsonb not null check (jsonb_typeof(splits) = 'array' and jsonb_array_length(splits) = 7),
  created_at timestamptz not null default now()
);
